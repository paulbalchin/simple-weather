...
export function Evening({ n, forecast, label }: { n: number; forecast: any; label: string }) {
  const q = forecast;
  const later = n === 0 ? "this evening" : "later this evening";
  ...
  return (
    <p className={`${classNames} ${classNames}__afternoon`}>
      <Unmask label={`${description} and ${temperature}° ${later}.`} />
      {rain && (
        <>
          {" "}
          <mark>
